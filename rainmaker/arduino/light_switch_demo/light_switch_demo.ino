...
static int gpio_switch = 14;
static int gpio_light = 21;
...
static LightBulb my_light("Lightbulb", &gpio_light);
...
void write_callback(Device *device, Param *param, const param_val_t val, void *priv_data, write_ctx_t *ctx)
{
    ...
    if(strcmp(device_name, "Lightbulb") == 0) {
      ...
      if(strcmp(param_name, "Power") == 0) {
          ...
          (light_state == false) ? digitalWrite(gpio_light, LOW) : digitalWrite(gpio_light, HIGH);
...
{
    Serial.begin(115200);
    pinMode(gpio_manual, INPUT);
    pinMode(gpio_switch, OUTPUT);
    pinMode(gpio_light, OUTPUT);
    digitalWrite(gpio_switch, DEFAULT_POWER_MODE);
    digitalWrite(gpio_light, DEFAULT_POWER_MODE);
...
void loop()
{
    if(digitalRead(gpio_manual) == LOW) { //Push button pressed

        // Key debounce handling
        delay(100);
        int startTime = millis();
        while(digitalRead(gpio_manual) == LOW) delay(50);
        int endTime = millis();
